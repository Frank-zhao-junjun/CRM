...
### 19. 数据分析与 BI 仪表盘 (V5.0 NEW)
- **BI 综合仪表盘** (/reports/bi)
  - KPI 总览（管道金额、成交金额、赢单率、预测准确度）
  - 趋势分析（近 6 个周期成交/管道对比）
  - 漏斗阶段分析（阶段金额、数量、转化率）
  - 团队排行（成交金额、赢单数、转化率）
- **报表 API 增强** (/api/reports?type=bi)
  - 统一输出 BI 数据模型（kpi/trend/funnel/ranking/conversion）
  - 支持时间范围筛选（month/quarter/year/all）
...
- **新增数据分析与 BI 仪表盘**
  - 新增 BI 页面：`/reports/bi`
  - 新增 BI API：`GET /api/reports?type=bi`
  - 补齐报表数据层函数：统计、漏斗、团队排行
  - 报表中心新增 BI 入口卡片
