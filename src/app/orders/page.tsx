...
import { Plus, Search, Package, Trash2, MoreVertical, X, CheckCircle, XCircle, ArrowRight } from 'lucide-react';
...
export default function OrdersPage() {
  ...
  const handleCreate = async () => {
    ...
    const totalDiscount = createItems.reduce((sum, i) => sum + i.discount, 0);
    const tax = (subtotal - totalDiscount) * 0.06;
    ...
    try {
      ...
      if (res.ok) {
        setShowCreate(false);
        setCreateForm({ opportunityId: '', customerId: '', customerName: '', orderDate: new Date().toISOString().split('T')[0], deliveryDate: '', paymentMethod: 'bank_transfer', notes: '' });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="section-title text-xl">成交订单</h1>
