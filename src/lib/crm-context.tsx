...
export function CRMProvider({ children }: { children: ReactNode }) {
  ...
  const addPaymentPlan = useCallback(async (plan: Omit<PaymentPlan, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    addActivity('created', 'payment_plan', newPlan.id, newPlan.title, `创建回款计划 "${newPlan.title}"`);
  }, [addActivity]);
  ...
  const updatePaymentPlan = useCallback(async (id: string, updates: Partial<PaymentPlan>) => {
    ...
    if (plan) {
      addActivity('updated', 'payment_plan', id, plan.title, `更新回款计划 ${plan.title}`);
    }
  ...
  const deletePaymentPlan = useCallback(async (id: string) => {
    ...
    if (plan) {
      addActivity('deleted', 'payment_plan', id, plan.title, `删除回款计划 ${plan.title}`);
    }
  }, [paymentPlans, addActivity]);
  ...
  const recordPayment = useCallback(async (planId: string, amount: number, method?: string) => {
    ...
    addActivity('updated', 'payment_plan', planId, plan.title, `登记回款 ¥${amount.toLocaleString()}`);
  ...
  const addTask = useCallback(async (task: Omit<Task, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    addActivity('created', 'task', newTask.id, newTask.title, `创建任务 "${newTask.title}"`);
  }, [addActivity]);
  ...
  const updateTask = useCallback(async (id: string, updates: Partial<Task>) => {
    ...
    if (task) {
      addActivity('updated', 'task', id, task.title, `更新任务 ${task.title}`);
    }
  }, [tasks, addActivity]);
  ...
  const deleteTask = useCallback(async (id: string) => {
    ...
    if (task) {
      addActivity('deleted', 'task', id, task.title, `删除任务 ${task.title}`);
    }
  }, [tasks, addActivity]);
  ...
  const completeTask = useCallback(async (id: string) => {
    ...
    await apiPut('updateTask', id, updated);
    setTasks(prev => prev.map(t => t.id === id ? { ...t, ...updated } : t));
    addActivity('updated', 'task', id, task.title, `完成任务 "${task.title}"`);
  }, [tasks, addActivity]);
